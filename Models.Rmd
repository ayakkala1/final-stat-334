...
tidy(cooks.distance(model1)) %>%
  arrange(desc(x))
```

The mean GradRate in our dataset is 64. Using a rule of thumb of Cooks D > 3 times the mean is an outlier implies that none of our observations are influential observations.
...
We fail to reject the null hypothesis that there is no association for Gradrate and Full Time Undergraduate Students adjusted for the other predictor variabels and no association between Gradrate and Total Enrolled adjusted for the other predictor variables.

So we do not have enough evidence to not drop Full Time Undergraduate Studetns and Total Enrolled from our model.
...
model3_sum <- tidy(summary(model3))
...
```{r}
find_sig(model3_sum, 0.05, bonferonni = F)
```

We have an additional significant beta, and our new variable is significant

What is still not significant is SFRatio, Top10, and PhD
